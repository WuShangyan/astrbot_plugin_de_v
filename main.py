...
# Random epigraphs — one per skill, in that skill's inner voice. On /de off a
# random one is chosen as the banner's quote line, so the exit message varies
# each time. The skill name is deliberately NOT printed — only the line itself.
...
def _build_banner_off() -> str:
    """Assemble the off-banner with a randomly-chosen skill epigraph.

    A random inner voice gets the last word as DE mode shuts down; the skill
    name is deliberately not printed. The frame (bars + separator + the
    'stage lights off' line) stays fixed so the banner is still recognisable.
    """
    epigraph = random.choice(tuple(SKILL_EPIGRAPHS.values()))
    return (
        "░▒▓██████████████████████████████████▓▒░\n\n"
        f"{epigraph}\n\n"
        "------------------------------------------------------------------------\n"
...
class DEPlugin(Star):
    ...
    async def de_off(self, event: AstrMessageEvent):
        ...
        if self.config.get("banner_on_toggle", True):
            yield event.plain_result(_build_banner_off())
        else:
